...
import com.bingo.server.game.landlord.comparator.HexCardComparator;
import com.bingo.server.game.landlord.constant.FightConstant;
import com.bingo.server.game.landlord.entity.bo.Game;
import com.bingo.server.game.landlord.entity.po.*;
import com.bingo.server.game.landlord.entity.po.cardlist.*;
...
import com.bingo.server.game.landlord.scheduler.EventScheduler;
...
import com.bingo.server.game.landlord.util.base.RandomUtils;
import com.bingo.server.game.landlord.util.base.ReflectUtils;
import com.bingo.server.game.landlord.util.base.TimeUtils;
import com.bingo.server.gate.provider.NotificationProvider;
import com.bingo.server.msg.Entity.GameState;
import com.bingo.server.msg.Entity.PaiNum;
import com.bingo.server.msg.Entity.Record;
import com.bingo.server.msg.Error.ErrorCode;
import com.bingo.server.msg.Fight;
import com.bingo.server.msg.Fight.FightRecommandResponse.RecommandPai;
import com.bingo.server.msg.Fight.SCFightGameOver.GameOverData;
import com.bingo.server.msg.Fight.SCFightRoundOver.Score;
import com.bingo.server.msg.ServerMessage;
import com.bingo.server.msg.ServerMessage.SC;
...
@Service
public class FightService extends ObserveBaseService implements FightProvider {
        ...
        if (msg.equals("MINGPAI")) {
            ...
            if (roleId == 0) {
                ...
                SC sc = SC
                        .newBuilder()
                        .setSCFightMingPai(
                                Fight.SCFightMingPai.newBuilder().addAllPai(LandLord.cards)
                                        .setSeated(game.getRoleIdList().indexOf(LandLord.gameRoleId))).build();
                for (RoleGameInfo info : game.getRoleIdMap().values()) {
                    sendToService.send(info.roleId, sc);
                }
            }
    ...
    public void readyGame(Role role) {
            ...
            sendToService.send(
                    ...
                    SC.newBuilder()
                            .setFightReadyResponse(
                                    Fight.FightReadyResponse.newBuilder().setErrorCode(ErrorCode.GAME_NOT_EXIST.getNumber()))
                            .build());
            return;
        }

        String gameRoleId = matchService.getGameRoleId(game.getGameId(), role.getId());
        RoleGameInfo roleGameInfo = game.getRoleIdMap().get(gameRoleId);

        // 游戏准备
        sendToService.send(roleGameInfo.roleId, SC.newBuilder().setFightReadyResponse(Fight.FightReadyResponse.newBuilder())
                .build());
        ...
        SC scFightReady = SC
                .newBuilder()
                .setSCFightReady(
                        Fight.SCFightReady.newBuilder().setSeated(game.getRoleIdList().indexOf(roleGameInfo.gameRoleId)))
                .build();
        for (RoleGameInfo info : game.getRoleIdMap().values())
            sendToService.send(info.roleId, scFightReady);
        ...
        if (this.checkAllReady(role.getGameId())) {
            ...
            Fight.SCFightStart.Builder FSBuilder = Fight.SCFightStart.newBuilder();
            ...
            for (RoleGameInfo info : game.getRoleIdMap().values()) {
                ...
                sendToService.send(info.roleId, SC.newBuilder().setSCFightStart(FSBuilder.clone().addAllPai(info.cards))
                        .build());
                sendToService.send(
                        info.roleId,
                        SC.newBuilder()
                                .setSCFightCallLandLord(
                                        Fight.SCFightCallLandLord.newBuilder().setCurrentFen(0)
     ...
     */
    private boolean checkAllReady(long gameId) {
     ...
     * @author wcy 2017年5月31日
     */
    private void gameInit(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
        game.setMultiple(0);
    ...
    public void mingPai(Role role) {
        ...
        if (game == null) {
            sendToService.send(role.getId(),
                    SC.newBuilder().setFightMingPaiResponse(
                            Fight.FightMingPaiResponse.newBuilder()
                                    .setErrorCode(ErrorCode.GAME_NOT_EXIST.getNumber()))
                            .build());
        }
        RoleGameInfo LandLord = game.getRoleIdMap().get(game.getLandlordGameRoleId());
        if (game.getRoleIdMap().get(game.getLandlordGameRoleId()).roleId != role.getId()) {
            sendToService.send(role.getId(),
                    SC.newBuilder().setFightMingPaiResponse(
                            Fight.FightMingPaiResponse.newBuilder()
        ...
        sendToService.send(role.getId(),
                SC.newBuilder().setFightMingPaiResponse(Fight.FightMingPaiResponse.newBuilder()).build());

    }

    @Override
    public ServerMessage.SC getLastRecord(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
        if (game == null) {
            return SC.newBuilder().setFightGetlastRoundResponse(Fight.FightGetlastRoundResponse.newBuilder().setErrorCode(ErrorCode.GAME_NOT_EXIST.getNumber())).build();
        }
        List<List<CardRecord>> records = game.getRecords().get(game.getRecords().size() - 1);
        if (records.size() < 2) {
            return SC.newBuilder().setFightGetlastRoundResponse(Fight.FightGetlastRoundResponse.newBuilder().setErrorCode(ErrorCode.FIRST_ROUND.getNumber())).build();
        }
        Fight.FightGetlastRoundResponse.Builder builder = Fight.FightGetlastRoundResponse.newBuilder();
    ...
    @Override
    public ServerMessage.SC exitGame(Role role) {
        Game game = GameCache.getGameMap().get(role.getGameId());
        if (game == null) {
            return SC
                    .newBuilder()
                    .setFightExitGameResponse(
                            Fight.FightExitGameResponse.newBuilder().setErrorCode(ErrorCode.GAME_NOT_EXIST.getNumber()))
                    .build();
        }
        ...
        if (gameState == GameState.GAME_STATE_PREPARE && game.getRound() == game.getGameConfig().getRound()) {
            // 若是房主，则直接解散
            if (game.getMasterRoleId() == role.getId()) {
                game.setGameState(GameState.GAME_START_END);

                SC scDismiss = SC.newBuilder().setSCFightGameDismiss(Fight.SCFightGameDismiss.newBuilder()).build();
                for (RoleGameInfo info : game.getRoleIdMap().values())
                    sendToService.send(info.roleId, scDismiss);
                GameCache.getGameLockStringMap().remove(game.getLockString());
                // 将游戏从缓存池中移除
                GameCache.getGameMap().remove(game.getGameId());
            } else {
                // 该玩家退出
                SC scExit = SC.newBuilder().setSCFightExitGame(Fight.SCFightExitGame.newBuilder().setGameRoleId(gameRoleId))
                        .build();
                for (RoleGameInfo info : game.getRoleIdMap().values())
                    sendToService.send(info.roleId, scExit);
        ...
        // 如果游戏已经开始,则要申请退出
            ...
            if (game.getOnlineRoleCount() != 0) {
                return SC
                        .newBuilder()
                        .setFightExitGameResponse(
                                Fight.FightExitGameResponse.newBuilder().setErrorCode(ErrorCode.GAME_EXITING.getNumber()))
                        .build();
            }
            SC scApplyExit = SC
                    .newBuilder()
                    .setSCFightApplyExitGame(
                            Fight.SCFightApplyExitGame.newBuilder().setName(role.getUsername())
                                    .setCountDown(FightConstant.COUNTDOWN)).build();
            // setOnlineCount(game.getGameId());
            for (RoleGameInfo info : game.getRoleIdMap().values()) {
                if (info.roleId != role.getId()) {
                    sendToService.send(info.roleId, scApplyExit);
        ...
        return SC.newBuilder().setFightExitGameResponse(Fight.FightExitGameResponse.newBuilder()).build();
    }

    /*private void setOnlineCount(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
        for (RoleGameInfo info : game.getRoleIdMap().values()) {
    ...
    @Override
    public ServerMessage.SC agreeExit(Role role, boolean agree) {
        System.out.println(role.getId() + "" + agree);
        Game game = GameCache.getGameMap().get(role.getGameId());
        if (game == null) {
            return SC
                    .newBuilder()
                    .setFightAgreeExitGameResponse(
                            Fight.FightAgreeExitGameResponse.newBuilder().setErrorCode(ErrorCode.GAME_NOT_EXIST.getNumber()))
                    .build();
        } else {
            String roleInfoStr = matchService.getGameRoleId(game.getGameId(), role.getId());
            return agreeExit(game.getGameId(), roleInfoStr, agree);
        }
    }

    public ServerMessage.SC agreeExit(long gameId, String exitGameRoleId, boolean agree) {
        Game game = GameCache.getGameMap().get(gameId);
        RoleGameInfo roleInfo = game.getRoleIdMap().get(exitGameRoleId);
        synchronized (game) {
            roleInfo.agreeLeave = agree;
            if (game.getOnlineRoleCount() == 0) {
                return SC.newBuilder().setFightAgreeExitGameResponse(Fight.FightAgreeExitGameResponse.newBuilder()).build();
            }
            int flag = 0;
            ...
            if (flag == -1) {
                for (RoleGameInfo roleGameInfo : game.getRoleIdMap().values()) {
                    sendToService.send(roleGameInfo.roleId,
                            SC.newBuilder()
                                    .setSCAgreeExitGame(Fight.SCAgreeExitGame.newBuilder()
                                            .setName(rejectRoleGameInfo.roleId == 0
                                                    ? "ROBOT" + rejectRoleGameInfo.gameRoleId
                                                    : RoleCache.getRoleById(rejectRoleGameInfo.roleId).getUsername()))
                                    .build());
                }
                return SC.newBuilder().setFightAgreeExitGameResponse(
                        Fight.FightAgreeExitGameResponse.newBuilder().setErrorCode(ErrorCode.APPLY_REJECT.getNumber()))
                        .build();
    ...
    @Override
    public void dispatchCard(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
        for (RoleGameInfo info : game.getRoleIdMap().values())
            info.cards.clear();
        ...
        for (int j = 0; j < needCard; j++) {
            ...
            for (RoleGameInfo roleGameInfo : game.getRoleIdMap().values()) {
                ...
                logger.info(roleGameInfo.roleId + " : " + roleGameInfo.cards);
            }
            ...
            sendToService.send(
                    ...
                    SC.newBuilder()
                            .setFightCallLandLordResponse(
                                    Fight.FightCallLandLordResponse.newBuilder().setErrorCode(
                                            ErrorCode.GAME_NOT_EXIST.getNumber())).build());
        }
        String gameRoleId = matchService.getGameRoleId(game.getGameId(), role.getId());
        sendToService.send(role.getId(),
                SC.newBuilder().setFightCallLandLordResponse(Fight.FightCallLandLordResponse.newBuilder()).build());
        // 叫地主
    ...
    @Override
    public ServerMessage.SC recommandCardList(Role role) {
        long gameId = role.getGameId();
        if (role.getId() != this.getCurrentRoleGameInfo(gameId).roleId) {
            return SC.newBuilder().setFightRecommandResponse(
                    Fight.FightRecommandResponse.newBuilder()
                            .setErrorCode(ErrorCode.NOT_YOUR_TURN.getNumber()))
                    .build();

        }
        List<List<Integer>> list = this.getRecommandCardList(gameId);
        Fight.FightRecommandResponse.Builder builder = Fight.FightRecommandResponse.newBuilder();
     ...
     */
    private List<List<Integer>> getRecommandCardList(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
    ...
    private void callLandlord(long gameId, String gameRoleId, int callScore) {
        Game game = GameCache.getGameMap().get(gameId);
        ...
        if (call) {
            ...
            if (game.getCallLandlordScore() == FightConstant.SCORE_3) {
                ...
                Fight.SCFightLandLord scFightLandLord = Fight.SCFightLandLord.newBuilder()
                ...
                for (RoleGameInfo info : game.getRoleIdMap().values()) {
                    if (!info.gameRoleId.equals(game.getLandlordGameRoleId())) {
                        sendToService.send(info.roleId, SC.newBuilder().setSCFightLandLord(scFightLandLord).build());
                    } else {
                        sendToService.send(
                                info.roleId,
                                SC.newBuilder()
                    ...
                    sendToService.send(
                            ...
                            SC.newBuilder()
                                    .setSCFightPutOut(
                                            Fight.SCFightPutOut.newBuilder().setCountdown(FightConstant.COUNTDOWN)
        ...
        if (callCount >= game.getMaxRoleCount()) {
            ...
            if (resultScore == 0) {
                ...
                for (RoleGameInfo info : game.getRoleIdMap().values()) {
                    System.out.println("~~~" + info.roleId + game.getMultiple());
                    sendToService.send(info.roleId,
                ...
                Fight.SCFightLandLord scFightLandLord = Fight.SCFightLandLord.newBuilder().addAllLandLordPai(game.getLandlordCards())
                        .setSeated(game.getRoleIdList().indexOf(game.getLandlordGameRoleId())).build();
                for (RoleGameInfo info : game.getRoleIdMap().values()) {
                    if (!info.gameRoleId.equals(game.getLandlordGameRoleId())) {
                        sendToService.send(info.roleId, SC.newBuilder().setSCFightLandLord(scFightLandLord).build());
            ...
            long roleId = game.getRoleIdMap().get(nextRoleGameId).roleId;
            game.setCallLandlordCount(callCount);
            for (RoleGameInfo info : game.getRoleIdMap().values()) {
                sendToService.send(
                        info.roleId,
                        SC.newBuilder()
                                .setSCFightCallLandLord(
                                        Fight.SCFightCallLandLord.newBuilder().setCurrentFen(game.getCallLandlordScore())
     ...
     */
    private void giveLandlordCards(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
            ...
            sendToService.send(
                    ...
                    SC.newBuilder()
                            .setFightSendCardResponse(
                                    Fight.FightSendCardResponse.newBuilder().setErrorCode(
                                            ErrorCode.GAME_NOT_EXIST.getNumber())).build());
            return;
        }
        String gameRoleId = game.getRoleIdList().get(game.getCurrentRoleIdIndex());
        RoleGameInfo roleGameInfo = game.getRoleIdMap().get(gameRoleId);
        if (roleGameInfo.roleId != role.getId()) {
            sendToService.send(
                    role.getId(),
                    SC.newBuilder()
                            .setFightSendCardResponse(
                                    Fight.FightSendCardResponse.newBuilder()
    ...
    private void gameRoleIdSendCard(List<Integer> paiList, long gameId, String gameRoleId) {
        Game game = GameCache.getGameMap().get(gameId);
        long roleId = game.getRoleIdMap().get(gameRoleId).roleId;
...
//		game.getCurrentRecord().add();
        ...
        sendToService.send(roleId, SC.newBuilder().setFightSendCardResponse(Fight.FightSendCardResponse.newBuilder()).build());
        /**
         * 通知已经出牌
         */
        Fight.SCFightPutOutPai.Builder SCPutOutBuilder = Fight.SCFightPutOutPai.newBuilder().addAllPutOutPai(paiList)
                .setSeated(game.getCurrentRoleIdIndex())
        ...
        Fight.SCFightPutOutPai sCFightPutOutPai = SCPutOutBuilder.build();
        for (RoleGameInfo tInfo : game.getRoleIdMap().values()) {
            sendToService.send(
                    tInfo.roleId,
                    SC.newBuilder()
    ...
    private void gameOver(long gameId, String gameRoleId) {
        Game game = GameCache.getGameMap().get(gameId);
        if (game == null) {
            return;
        }
        ...
        Fight.SCFightRoundOver.Builder builder = Fight.SCFightRoundOver.newBuilder();
        ...
        if (game.getRound() == 0) {
            //TODO 解散
            Fight.SCFightGameOver.Builder SCGameOverBuilder = Fight.SCFightGameOver.newBuilder();
            for (RoleGameInfo info : game.getRoleIdMap().values()) {
            ...
            for (RoleGameInfo info : game.getRoleIdMap().values()) {
                sendToService.send(info.roleId, gameOverSC);
                sendToService.send(info.roleId, SC.newBuilder().setSCFightGameDismiss(Fight.SCFightGameDismiss.newBuilder()).build());
            }
            GameCache.getGameLockStringMap().remove(game.getLockString());
            GameCache.getGameMap().remove(game.getGameId());
        }
    ...
    private void checkAutoAI(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
        // 发送等待消息
     ...
     * @author wcy 2017年6月2日
     */
    private void autoSendCard(long gameId, String gameRoleId) {
        // 否则进行自动出牌
     ...
     * @author wcy 2017年6月2日
     */
    private List<Integer> getAutoPaiList(long gameId) {
    ...
    private int getNextIndex(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
    ...
    public void getBestCardList(Role role) {
        ...
        // 获得该玩家信息
        String gameRoleId = matchService.getGameRoleId(gameId, role.getId());
        RoleGameInfo roleGameInfo = game.getRoleIdMap().get(gameRoleId);
     ...
     * @author wcy 2017年6月2日
     */
    private RoleGameInfo getCurrentRoleGameInfo(long gameId) {
        Game game = GameCache.getGameMap().get(gameId);
        int index = game.getCurrentRoleIdIndex();
    ...
    private static void test_dispatchCard() {
        FightService impl = new FightService();
        Game game = new Game();
        game.setGameId(1);
